...
<meta http-equiv="Content-Type" content="text/html; charset=utf-8"/>
    ...
    <style>
      ...
      .ui-bank-record td {height: 28px; line-height: 28px; border: 1px solid #c6ab7c; background: #FFEAC2;}
        ...
        <asp:Repeater ID="rptInsureList" runat="server">
            <ItemTemplate>
                <tr>
                    ...
                    <td>
                        <%# GetNickNameByUserID(Convert.ToInt32(Eval("SourceUserID")))%>
                    </td>
                    <td>
                        <%# GetNickNameByUserID(Convert.ToInt32(Eval("TargetUserID")))%>
                    </td>
                    <td <%# Convert.ToInt32( Eval( "TradeType" ) ) == 1 ? "class=\"ui-color-green\"" : Convert.ToInt32( Eval( "TradeType" ) ) == 2 ? "class=\"ui-color-red\"" : "" %>>
                        <%# ( Convert.ToInt32( Eval( "TradeType" ) ) == 1 ? "存款" : Convert.ToInt32( Eval( "TradeType" ) ) == 2 ? "取款" : Convert.ToInt32( Eval( "TradeType" ) ) == 3 && int.Parse( Eval( "TargetUserID" ).ToString() ) != Game.Facade.Fetch.GetUserCookie().UserID ? "转出" : "转入" )%>
                    </td>
                    <td class="ui-font-weight">
                        <%# ( int.Parse( Eval( "TradeType" ).ToString( ) ) == 3 && int.Parse( Eval( "TargetUserID" ).ToString( ) ) != Game.Facade.Fetch.GetUserCookie( ).UserID ) ? ( 0 - double.Parse( Eval( "SwapScore" ).ToString( ) ) ) : double.Parse( Eval( "SwapScore" ).ToString( ) )%>
                    </td>
