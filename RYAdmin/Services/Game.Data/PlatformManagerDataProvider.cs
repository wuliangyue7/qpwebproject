...
namespace Game.Data
{
    ...
    public class PlatformManagerDataProvider : BaseDataProvider, IPlatformManagerDataProvider
    {
        ...
        public void Register(Base_Users user)
        {
            ...
            dr[Base_Users._IsAssist] = user.IsAssist;
        ...
        {
            string sqlQuery = "SELECT * FROM Base_Module WHERE ParentID=0 AND Nullity=0 ORDER BY OrderNo";
        ...
        {
            var prams = new List<DbParameter>();
            prams.Add(Database.MakeInParam("strAccounts", user.Username));
            prams.Add(Database.MakeInParam("strPassword", user.Password));
            prams.Add(Database.MakeInParam("strClientIP", user.LastLoginIP));
            prams.Add(Database.MakeInParam("strMachineSerial", machineID));
            prams.Add(Database.MakeOutParam("strErrorDescribe", typeof(string), 127));

            Message msg = MessageHelper.GetMessageForObject<Base_Users>(Database, "APP_PM_UserLogon", prams);
            return msg;
        }
        #endregion
    }
